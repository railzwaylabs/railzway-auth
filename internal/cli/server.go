...
func runServer() {
	app := fx.New(
		...
		fx.Invoke(useTelemetry, bootstrap.EnsureOrg, startHTTPServer),
	)

	app.Run()
}

// Dependency constructors copied/moved from main.go
...
func newOrgRepository(pool *pgxpool.Pool, q *sqlc.Queries) repository.OrgRepository {
	return repository.NewPostgresOrgRepo(pool, q)
}
